...
Both \rpack{RcppTN} and \rpack{truncnorm} use compiled code for their RNG. However,
the RNG in \rpack{msm} is written in \R{}. As a result, the performance cost
...
are over 20 times faster in drawing samples of size 1,000
...
above at 4.1. The sample size is still 1,000.
...
<<tidy=FALSE, echo = FALSE, cache = USECACHE>>=
